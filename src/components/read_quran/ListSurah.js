      ...
      <div className="h-full w-full p-4 overflow-y-auto">
        {listSurah && Array.isArray(listSurah) && listSurah.length > 0 ? (
          listSurah.map((surah, index) => (
            <ItemSurah
              key={surah.nomor} // Jangan lupa tambahkan key yang unik
              nomor={surah.nomor}
          ...
          <p>No Surah available</p> // Pesan yang muncul jika listSurah tidak tersedia
        )}
      </div>
    );
  }  
    ...
    return (
        <div className="h-24 w-full border-b-2 border-teal-700 flex mb-4">
            ...
            <div className="flex flex-col">
                ...
                <span className="text-base font-bold text-gray-500 ">{tempatTurunSurah}, {jumlahAyat} ayat</span>    
            </div>
        </div>
    )
}